function simulate_platoon_split_merge()
% 4-vehicle platoon vs a MOVING OV with CRPF + VO + DWA (longitudinal only).
...
cols = lines(nSV); for i=1:nSV, SV(i).color = cols(i,:); end
...
for k=1:N
  ...
  % --- SIDE DECISION (which lane to be on) + start lane change if needed
  for i=1:nSV
      % Honor an ongoing lane-change
      if SV(i).lc_active
          % keep target as chosen; we’ll advance LC below
      ...
      if need_consider
          ...
          if center_conflict
              ...
              elseif freeL && freeR
                  % pick side farther from OV.x (tie-break with light balancing)
                  clrL = abs(ov_x - lane_center_left);
                  clrR = abs(ov_x - lane_center_right);
                  next_lane = ternary(clrR >= clrL, "RIGHT", "LEFT");
                  % anti-herding among nearby SVs
                  ...
                  if next_lane=="LEFT" && right_count < left_count,  next_lane="RIGHT"; end
                  if next_lane=="RIGHT" && left_count  < right_count, next_lane="LEFT";  end
              end

              % start lane change if it actually changes lane
              if next_lane ~= "MAIN"
  ...
  for i=1:nSV
      ...
      % Overtake floor if behind and on a passing lane (either actively changing to, or sitting on, LEFT/RIGHT)
...
%% --- Heatmap anchored at last leave-from-MAIN (unchanged from your flow)
...
title('CRPF Heatmap @ OV pose when last SV starts lane change'); xlabel('X [m]'); ylabel('Y [m]');
...
legend({'SV1','SV2','SV3','SV4','OV (moving)'}, 'Location','best');
